...
export default function Relatorios() {
    ...
    useEffect(() => {
        const filtered = movimentacoes.filter((mov) => {
            const dateInRange = (!filters.startDate || mov.date >= filters.startDate) &&
        ...
        datasets: [
            {
                label: 'Valor Total',
                data: [
                    filteredMovimentacoes.filter(mov => mov.type === 'receita').reduce((sum, mov) => sum + mov.value, 0),
                    filteredMovimentacoes.filter(mov => mov.type === 'despesa_fixa').reduce((sum, mov) => sum + mov.value, 0),
                    filteredMovimentacoes.filter(mov => mov.type === 'despesa_variavel').reduce((sum, mov) => sum + mov.value, 0)
                ],
